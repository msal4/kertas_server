...
func (sq *ScheduleQuery) sqlAll(ctx context.Context) ([]*Schedule, error) {
	...
	if query := sq.withClass; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "class_schedules" returned %v`, n.ID)
			}
			for i := range nodes {
