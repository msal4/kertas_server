...
func (nq *NotificationQuery) QueryStage() *StageQuery {
	query := &StageQuery{config: nq.config}
...
func (nq *NotificationQuery) WithStage(opts ...func(*StageQuery)) *NotificationQuery {
	query := &StageQuery{config: nq.config}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context) ([]*Notification, error) {
	...
	if query := nq.withStage; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Notification)
		for i := range nodes {
			if nodes[i].stage_notifications == nil {
				continue
			}
			fk := *nodes[i].stage_notifications
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "stage_notifications" returned %v`, n.ID)
			}
			for i := range nodes {
