...
func (cq *ClassQuery) QueryStage() *StageQuery {
	query := &StageQuery{config: cq.config}
...
func (cq *ClassQuery) QueryTeacher() *UserQuery {
...
func (cq *ClassQuery) QueryGroup() *GroupQuery {
	query := &GroupQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ClassQuery) QueryAssignments() *AssignmentQuery {
	query := &AssignmentQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ClassQuery) QueryAttendances() *AttendanceQuery {
	query := &AttendanceQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ClassQuery) QuerySchedules() *ScheduleQuery {
...
func (cq *ClassQuery) WithStage(opts ...func(*StageQuery)) *ClassQuery {
	query := &StageQuery{config: cq.config}
...
func (cq *ClassQuery) WithTeacher(opts ...func(*UserQuery)) *ClassQuery {
	query := &UserQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ClassQuery) WithGroup(opts ...func(*GroupQuery)) *ClassQuery {
	query := &GroupQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ClassQuery) WithAssignments(opts ...func(*AssignmentQuery)) *ClassQuery {
	query := &AssignmentQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withAssignments = query
	return cq
}
...
func (cq *ClassQuery) WithAttendances(opts ...func(*AttendanceQuery)) *ClassQuery {
	query := &AttendanceQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withAttendances = query
	return cq
}
...
func (cq *ClassQuery) WithSchedules(opts ...func(*ScheduleQuery)) *ClassQuery {
	query := &ScheduleQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withSchedules = query
	return cq
}
...
func (cq *ClassQuery) sqlAll(ctx context.Context) ([]*Class, error) {
	...
	if query := cq.withStage; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "stage_classes" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withTeacher; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_classes" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withGroup; query != nil {
		...
		query.Where(predicate.Group(func(s *sql.Selector) {
			s.Where(sql.InValues(class.GroupColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.class_group
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "class_group" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "class_group" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Group = n
		}
	}
	...
	if query := cq.withAssignments; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Assignments = []*Assignment{}
		}
		query.withFKs = true
		query.Where(predicate.Assignment(func(s *sql.Selector) {
			s.Where(sql.InValues(class.AssignmentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.class_assignments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "class_assignments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "class_assignments" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Assignments = append(node.Edges.Assignments, n)
		}
	}
	...
	if query := cq.withAttendances; query != nil {
		...
		query.Where(predicate.Attendance(func(s *sql.Selector) {
			s.Where(sql.InValues(class.AttendancesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.class_attendances
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "class_attendances" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "class_attendances" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := cq.withSchedules; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Schedules = []*Schedule{}
		}
		query.withFKs = true
		query.Where(predicate.Schedule(func(s *sql.Selector) {
			s.Where(sql.InValues(class.SchedulesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.class_schedules
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "class_schedules" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
