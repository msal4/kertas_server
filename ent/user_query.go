...
func (uq *UserQuery) QueryStage() *StageQuery {
	query := &StageQuery{config: uq.config}
...
func (uq *UserQuery) QuerySchool() *SchoolQuery {
	query := &SchoolQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryClasses() *ClassQuery {
	query := &ClassQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QuerySubmissions() *AssignmentSubmissionQuery {
	query := &AssignmentSubmissionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryAttendances() *AttendanceQuery {
	query := &AttendanceQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryPayments() *TuitionPaymentQuery {
	query := &TuitionPaymentQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryGrades() *GradeQuery {
	query := &GradeQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithStage(opts ...func(*StageQuery)) *UserQuery {
	query := &StageQuery{config: uq.config}
...
func (uq *UserQuery) WithSubmissions(opts ...func(*AssignmentSubmissionQuery)) *UserQuery {
	query := &AssignmentSubmissionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithAttendances(opts ...func(*AttendanceQuery)) *UserQuery {
	query := &AttendanceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withAttendances = query
	return uq
}
...
func (uq *UserQuery) WithPayments(opts ...func(*TuitionPaymentQuery)) *UserQuery {
	query := &TuitionPaymentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithGrades(opts ...func(*GradeQuery)) *UserQuery {
	query := &GradeQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withStage; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "stage_students" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := uq.withSchool; query != nil {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "school_users" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := uq.withClasses; query != nil {
		...
		query.Where(predicate.Class(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ClassesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_classes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_classes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_classes" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withSubmissions; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Submissions = []*AssignmentSubmission{}
		}
		query.withFKs = true
		query.Where(predicate.AssignmentSubmission(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SubmissionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_submissions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
	...
	if query := uq.withAttendances; query != nil {
		...
		query.Where(predicate.Attendance(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AttendancesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_attendances" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_attendances" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withPayments; query != nil {
		...
		query.Where(predicate.TuitionPayment(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PaymentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_payments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_payments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_payments" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withGrades; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Grades = []*Grade{}
		}
		query.withFKs = true
		query.Where(predicate.Grade(func(s *sql.Selector) {
			s.Where(sql.InValues(user.GradesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_grades
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_grades" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_grades" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Grades = append(node.Edges.Grades, n)
		}
	}
