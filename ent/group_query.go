...
func (gq *GroupQuery) QueryClass() *ClassQuery {
...
func (gq *GroupQuery) WithClass(opts ...func(*ClassQuery)) *GroupQuery {
	query := &ClassQuery{config: gq.config}
...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if query := gq.withClass; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "class_group" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := gq.withMessages; query != nil {
		...
		for _, n := range neighbors {
			fk := n.group_messages
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "group_messages" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "group_messages" returned %v for node %v`, *fk, n.ID)
			}
