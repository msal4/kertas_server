...
// TimeMixinEdge is the edge representation of TimeMixin.
type TimeMixinEdge struct {
	Node   *TimeMixin `json:"node"`
	Cursor Cursor     `json:"cursor"`
}

// TimeMixinConnection is the connection containing edges to TimeMixin.
type TimeMixinConnection struct {
	Edges      []*TimeMixinEdge `json:"edges"`
	PageInfo   PageInfo         `json:"pageInfo"`
	TotalCount int              `json:"totalCount"`
}

// TimeMixinPaginateOption enables pagination customization.
type TimeMixinPaginateOption func(*timeMixinPager) error

// WithTimeMixinOrder configures pagination ordering.
func WithTimeMixinOrder(order *TimeMixinOrder) TimeMixinPaginateOption {
	if order == nil {
		order = DefaultTimeMixinOrder
	}
	o := *order
	return func(pager *timeMixinPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultTimeMixinOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithTimeMixinFilter configures pagination filter.
func WithTimeMixinFilter(filter func(*TimeMixinQuery) (*TimeMixinQuery, error)) TimeMixinPaginateOption {
	return func(pager *timeMixinPager) error {
		if filter == nil {
			return errors.New("TimeMixinQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type timeMixinPager struct {
	order  *TimeMixinOrder
	filter func(*TimeMixinQuery) (*TimeMixinQuery, error)
}

func newTimeMixinPager(opts []TimeMixinPaginateOption) (*timeMixinPager, error) {
	pager := &timeMixinPager{}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultTimeMixinOrder
	}
	return pager, nil
}

func (p *timeMixinPager) applyFilter(query *TimeMixinQuery) (*TimeMixinQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *timeMixinPager) toCursor(tm *TimeMixin) Cursor {
	return p.order.Field.toCursor(tm)
}

func (p *timeMixinPager) applyCursors(query *TimeMixinQuery, after, before *Cursor) *TimeMixinQuery {
	for _, predicate := range cursorsToPredicates(
		p.order.Direction, after, before,
		p.order.Field.field, DefaultTimeMixinOrder.Field.field,
	) {
		query = query.Where(predicate)
	}
	return query
...
func (p *timeMixinPager) applyOrder(query *TimeMixinQuery, reverse bool) *TimeMixinQuery {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	query = query.Order(direction.orderFunc(p.order.Field.field))
	if p.order.Field != DefaultTimeMixinOrder.Field {
		query = query.Order(direction.orderFunc(DefaultTimeMixinOrder.Field.field))
	}
	return query
}

// Paginate executes the query and returns a relay based cursor connection to TimeMixin.
func (tm *TimeMixinQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...TimeMixinPaginateOption,
) (*TimeMixinConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newTimeMixinPager(opts)
	if err != nil {
		return nil, err
	}
	...
	conn := &TimeMixinConnection{Edges: []*TimeMixinEdge{}}
	if !hasCollectedField(ctx, edgesField) || first != nil && *first == 0 || last != nil && *last == 0 {
		if hasCollectedField(ctx, totalCountField) ||
			hasCollectedField(ctx, pageInfoField) {
	...
	if field := getCollectedField(ctx, edgesField, nodeField); field != nil {
		tm = tm.collectField(graphql.GetOperationContext(ctx), *field)
	}
	...
	var nodeAt func(int) *TimeMixin
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *TimeMixin {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *TimeMixin {
	...
	conn.Edges = make([]*TimeMixinEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &TimeMixinEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
var (
	// TimeMixinOrderFieldCreatedAt orders TimeMixin by created_at.
	TimeMixinOrderFieldCreatedAt = &TimeMixinOrderField{
		field: timemixin.FieldCreatedAt,
		toCursor: func(tm *TimeMixin) Cursor {
			return Cursor{
				ID:    tm.ID,
				Value: tm.CreatedAt,
			}
	...
	// TimeMixinOrderFieldUpdatedAt orders TimeMixin by updated_at.
	TimeMixinOrderFieldUpdatedAt = &TimeMixinOrderField{
		field: timemixin.FieldUpdatedAt,
		toCursor: func(tm *TimeMixin) Cursor {
			return Cursor{
				ID:    tm.ID,
				Value: tm.UpdatedAt,
...
// String implement fmt.Stringer interface.
func (f TimeMixinOrderField) String() string {
	var str string
	switch f.field {
...
// MarshalGQL implements graphql.Marshaler interface.
func (f TimeMixinOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *TimeMixinOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("TimeMixinOrderField %T must be a string", v)
	}
	switch str {
	case "CREATED_AT":
		*f = *TimeMixinOrderFieldCreatedAt
	case "UPDATED_AT":
		*f = *TimeMixinOrderFieldUpdatedAt
	default:
		return fmt.Errorf("%s is not a valid TimeMixinOrderField", str)
	}
	return nil
}

// TimeMixinOrderField defines the ordering field of TimeMixin.
type TimeMixinOrderField struct {
	field    string
	toCursor func(*TimeMixin) Cursor
}

// TimeMixinOrder defines the ordering of TimeMixin.
type TimeMixinOrder struct {
	Direction OrderDirection       `json:"direction"`
	Field     *TimeMixinOrderField `json:"field"`
}

// DefaultTimeMixinOrder is the default ordering of TimeMixin.
var DefaultTimeMixinOrder = &TimeMixinOrder{
	Direction: OrderDirectionAsc,
	Field: &TimeMixinOrderField{
		field: timemixin.FieldID,
		toCursor: func(tm *TimeMixin) Cursor {
...
// ToEdge converts TimeMixin into TimeMixinEdge.
func (tm *TimeMixin) ToEdge(order *TimeMixinOrder) *TimeMixinEdge {
	if order == nil {
		order = DefaultTimeMixinOrder
	}
	return &TimeMixinEdge{
		Node:   tm,
